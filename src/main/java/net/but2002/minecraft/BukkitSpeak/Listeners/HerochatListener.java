...
public class HerochatListener implements Listener {
	...
	public void onHeroChatMessage(ChannelChatEvent e) {
		...
		if (!hasPermission(e.getChatter().getPlayer(), "chat")) return;

		String channelName = e.getChannel().getName();

		if (e.getResult() == ChatResult.ALLOWED
				&& Configuration.PLUGINS_HEROCHAT_CHANNEL.getString().equalsIgnoreCase(channelName)) {
			String tsMsg = Messages.MC_EVENT_CHAT.get();
			tsMsg = new Replacer().addPlayer(e.getChatter().getPlayer()).addMessage(e.getMessage()).replace(tsMsg);
